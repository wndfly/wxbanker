...
from wxbanker.ObjectListView.WordWrapRenderer import WordWrapRenderer

#======================================================================

class OLVPrinter(wx.Printout):
    """
    An OLVPrinter creates a pretty report from an ObjectListView/ListCtrl.
    """

    def __init__(self, listCtrl=None, title="ObjectListView Printing"):
        """
        """
    ...
    def PrintPreview(self, parent=None, title="ObjectListView Print Preview", bounds=(20, 50, 800, 800)):
        ...
        #forPrinter.engine.listCtrls = list(self.engine.listCtrls)
        self.preview = wx.PrintPreview(self, forPrinter, data)
    ...
    def DoPrint(self, parent=None):
        ...
        printout.Destroy() #@UndefinedVariable
...
class ReportFormat(object):
    ...
    @staticmethod
    def Trying(fontName="Arial"):
        """
        Return a reasonable default format for a report
        """
        fmt = ReportFormat()
        fmt.PageHeader.Font = wx.FFont(12, wx.FONTFAMILY_DEFAULT, face=fontName)
        #fmt.PageHeader.Add(LineDecoration(pen=wx.Pen(wx.BLUE, 2), space=5))
        fmt.PageHeader.Background(wx.Colour(255, 255, 0)) # yellow
        fmt.PageHeader.Line(wx.TOP, wx.BLUE, 2, space=5)
        fmt.PageHeader.Line(wx.BOTTOM, wx.RED, 2, space=5)

        fmt.ReportHeader.Font = wx.FFont(24, wx.FONTFAMILY_DEFAULT, face=fontName)
        fmt.ReportHeader.TextColor = wx.RED
        fmt.ReportHeader.Padding = (0, 12, 0, 12)
        fmt.ReportHeader.Line(wx.BOTTOM, wx.GREEN, 5, wx.WHITE, space=5)

        fmt.ListHeader.Add(LineDecoration(side=wx.BOTTOM, pen=wx.Pen(wx.GREEN, 1)))

        fmt.PageFooter.Font = wx.FFont(12, wx.FONTFAMILY_DEFAULT, face=fontName)
        fmt.PageFooter.TextAlignment = wx.ALIGN_RIGHT
        fmt.PageFooter.Add(LineDecoration(side=wx.TOP, pen=wx.Pen(wx.BLUE, 1), space=3))
        ...
        #fmt.Row.Add(LineDecoration(side=wx.BOTTOM, pen=wx.Pen(wx.GREEN, 1)))
    ...
    def Normal(fontName="Arial"):
        ...
        fmt.ReportHeader.Font = wx.FFont(24, wx.FONTFAMILY_DEFAULT, face=fontName)
        fmt.ReportHeader.Padding = (0, 12, 0, 12)
        fmt.ReportHeader.TextAlignment = wx.ALIGN_CENTER
        fmt.ReportHeader.Frame(pen=wx.Pen(wx.BLUE, 2, wx.DOT), space=5)

        fmt.ListHeader.Font = wx.FFont(14, wx.FONTFAMILY_DEFAULT, face=fontName)
        fmt.ListHeader.Padding = (0, 12, 0, 12)
        fmt.ListHeader.Line(wx.BOTTOM, pen=wx.Pen(wx.BLUE, 2, wx.DOT), space=5)
        ...
        fmt.PageFooter.Font = wx.FFont(12, wx.FONTFAMILY_DEFAULT, face=fontName)
        fmt.PageFooter.TextAlignment = wx.ALIGN_RIGHT
        fmt.PageFooter.Line(wx.TOP, wx.BLUE, 1, space=3)

        fmt.ColumnHeader.Font = wx.FFont(12, wx.FONTFAMILY_DEFAULT, wx.FONTFLAG_BOLD, face=fontName)
        fmt.ColumnHeader.CellPadding = 5
        fmt.ColumnHeader.GridPen = wx.Pen(wx.Colour(192, 192, 192), 1)
        #fmt.ColumnHeader.Background(wx.Colour(128, 128, 128))

        fmt.Row.Font = wx.FFont(12, wx.FONTFAMILY_DEFAULT, face=fontName)
        fmt.Row.CellPadding = 5
        fmt.Row.GridPen = wx.Pen(wx.Colour(192, 192, 192), 1)
...
class BlockFormat(object):
    ...
    def Line(self, side=wx.BOTTOM, color=wx.BLACK, width=1, toColor=None, space=0, pen=None):
        ...
        if pen:
            ...
            self.Add(RectangleDecoration(side, pen, color, toColor, width, space))

    def Background(self, color=wx.BLUE, toColor=None):
        self.Add(RectangleDecoration(color=color, toColor=toColor))
...
class ReportBlock(Block):
    ...
    def Print(self, dc):
        ...
        first = True
        for (olv, title) in self.engine.listCtrls:
            if not first:
...
class ColumnHeaderBlock(CellBlock):
    ...
    def GetAlignments(self):
        ...
        if self.engine.alwaysCenterColumnHeader:
            return [wx.ALIGN_CENTRE for i in range(self.left, self.right+1)] #@UnusedVariable
        else:
            return self.GetColumnAlignments(olv, self.left, self.right) #@UndefinedVariable
...
class RectangleDecoration(Decoration):
    ...
    def __init__(self, side=None, pen=None, color=None, toColor=None, width=0, space=0):
        ...
        If no side is given, the rectangle will be drawn around the block.
        """
        self.side = side
        self.pen = pen
        self.color = color
    ...
    def SubtractFrom(self, dc, bounds):
        ...
        if self.side is None:       return RectUtils.InsetBy(bounds, inset)
...
if __name__ == '__main__':
    ...
    import ExampleModel #@UnresolvedImport
    import ExampleImages #@UnresolvedImport
    ...
    class MyFrame(wx.Frame):
        def __init__(self, *args, **kwds):
            ...
            userImage = self.olv.AddImages(ExampleImages.getUser16Bitmap(), ExampleImages.getUser32Bitmap()) #@UnusedVariable
        ...
        def run(self):
            ...
            printer.ReportFormat = ReportFormat.Normal("Chiller")
            #printer.ReportFormat = ReportFormat.Trying("Chiller")
