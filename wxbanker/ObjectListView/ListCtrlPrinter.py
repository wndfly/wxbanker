...
class ListCtrlPrintout(wx.Printout):
    ...
    def SetScaleAndBounds(self, dc):
        ...
        ppiPrinterX, ppiPrinterY = self.GetPPIPrinter() #@UnusedVariable
        ppiScreenX, ppiScreenY = self.GetPPIScreen() #@UnusedVariable
        logicalScale = float(ppiPrinterX) / float(ppiScreenX)
        pw, ph = self.GetPageSizePixels() #@UnusedVariable
        dw, dh = dc.GetSize()
...
if __name__ == '__main__':
    import wx #@Reimport
    from ObjectListView import ObjectListView, FastObjectListView, GroupListView, ColumnDefn #@UnusedImport
